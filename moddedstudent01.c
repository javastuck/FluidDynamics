...
//change04      
    ...
    tsmooth=0;
	omp_set_num_threads(4);
	#pragma omp parallel for reduction(+:tsmooth,cmass) shared(T) private(i,j,k) num_threads(4)
    for(i=1;i<=Nx;i++){
      for(j=1;j<=Ny;j++){
	for(k=1;k<=Nz;k++){
	  if(T[i][j][k]<0.0){
    ...
    temp=0.0;
	#pragma omp parallel for reduction(+:temp) shared(fph,ph) private(i,j,k) num_threads(4)
    for(i=1;i<=Nx;i++){
      for(j=1;j<=Ny;j++){
	for(k=1;k<=Nz;k++){
	  fph[i][j][k] = ph[i][j][k] *
	...
	#pragma omp parallel for shared(ph,fph) private(i,j,k) num_threads(4)
    for(i=1;i<=Nx;i++){
      for(j=1;j<=Ny;j++){
	for(k=1;k<=Nz;k++){
	  ph[i][j][k]=ph[i][j][k]
...
//change05
    #pragma omp parallel for shared(G,P,lam,mu,K,T,gam,divU,stressxx,ux,wz,vy,ph,stressyy,stresszz,stressxy,stressxz,stressyz,heatfluxx,heatfluxy,heatfluxz,vx,uy,wx,uz,wy,vz,Tx,Ty,Tz) private(i,j,k) num_threads(4)
    for(i=1;i<=Nx;i++){
      for(j=1;j<=Ny;j++){
	for(k=1;k<=Nz;k++){
